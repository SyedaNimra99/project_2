...
class Interface(ScreenManager):
    ...
    def fetching_data(self,dt):
        try:
            ...
            for key in keys:
                ...
                self.ids[key]=layout
                title=Custombtn(background_normal="orange.png",key_name=key,font_name="robotoblack.ttf",text=self.truncate_string(key , 10))
    ...
    def addItem(self,obj):
        self.popup.dismiss()
        layout=BoxLayout(spacing=dp(10),size_hint_y=None,height=dp(80))
        title=Custombtn(background_normal="orange.png",font_name="robotoblack.ttf",key_name=self.textInput.text ,text=self.truncate_string(self.textInput.text, 10))
        title.bind(on_press=self.detail_screen)
